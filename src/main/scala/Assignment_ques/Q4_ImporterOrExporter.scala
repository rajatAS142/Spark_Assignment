...
object ImporterOrExporter {
  ...
  def main(args: Array[String]) {
    ...
    val query_tool ="""
        ...
        from
        (
          select i.country ,i.year ,sum(i.value) as totalImport,sum(e.value) as totalExport
          from imported i inner join exported e
          on i.HSCode = e.HSCode
          group by i.country,i.year
        )
        order by country,year desc
      """

    println("Country and year wise Importer or Exporter:\n")
